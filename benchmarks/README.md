...
- `conditions` requires `fixture`, `provider`, `hermes_version`, `tools`, `temperature`, and `session_type`. The provenance fields are non-empty strings, `tools` is a non-empty list of non-empty strings, `temperature` is a finite number, and `session_type` must be `fresh-isolated`. For repository-based tasks, `fixture` pins the repository revision (for example `hermes-agent@<commit-sha>`); for non-repository tasks use a stable fixture id (for example `structured-data-v1`). Baseline and optimized runs are comparable only when their complete `conditions` objects are identical.
